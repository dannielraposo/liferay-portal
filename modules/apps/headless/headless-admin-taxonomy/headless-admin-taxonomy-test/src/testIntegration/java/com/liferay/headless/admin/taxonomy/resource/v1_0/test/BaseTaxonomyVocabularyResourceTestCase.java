...
public abstract class BaseTaxonomyVocabularyResourceTestCase {
	...
	public void testGraphQLGetSiteTaxonomyVocabulariesPage() throws Exception {
		...
		JSONObject siteTaxonomyVocabulariesJSONObject =
			JSONUtil.getValueAsJSONObject(
				invokeGraphQLQuery(graphQLField), "JSONObject/data",
				"JSONObject/siteTaxonomyVocabularies");

		long totalCount = siteTaxonomyVocabulariesJSONObject.getLong(
			"totalCount");
		...
		siteTaxonomyVocabulariesJSONObject = JSONUtil.getValueAsJSONObject(
			invokeGraphQLQuery(graphQLField), "JSONObject/data",
			"JSONObject/siteTaxonomyVocabularies");

		Assert.assertEquals(
			totalCount + 2,
			siteTaxonomyVocabulariesJSONObject.getLong("totalCount"));

		assertContains(
			taxonomyVocabulary1,
			Arrays.asList(
				TaxonomyVocabularySerDes.toDTOs(
					siteTaxonomyVocabulariesJSONObject.getString("items"))));
		assertContains(
			taxonomyVocabulary2,
			Arrays.asList(
				TaxonomyVocabularySerDes.toDTOs(
	...
	@Test
	public void testGraphQLPostTaxonomyVocabulary() throws Exception {
		TaxonomyVocabulary randomTaxonomyVocabulary =
			randomTaxonomyVocabulary();
			...
			JSONUtil.getValueAsString(
				invokeGraphQLMutation(
					new GraphQLField(
						...
						graphQLFields)),
				"JSONObject/data", "JSONObject/createTaxonomyVocabulary"),
			TaxonomyVocabulary.class);
	}
