...
public class ScheduledPublishProcessResourceImpl
	...
	public void deleteSiteScheduledPublishProcess(
			String siteExternalReferenceCode, String jobName)
		throws Exception {
		...
		if (schedulerResponse == null) {
			throw new NotFoundException(
				"No scheduled publish process was found with job name \"" +
					jobName + "\"");
		}

		_layoutService.unschedulePublishToLive(
			liveGroup.getGroupId(), jobName, groupName);
	}
	...
	public ScheduledPublishProcess getSiteScheduledPublishProcess(
		...
		return _toScheduledPublishProcess(schedulerResponse);
	}
		...
		return new ScheduledPublishProcess() {
			{
				...
				setCronExpression(
					() -> {
						...
						return GetterUtil.getString(
							ArrayUtil.getValue(
								parameterMap.get("cronExpression"), 0),
							null);
					});
				setDateCreated(
					() -> {
				...
				setJobName(schedulerResponse::getJobName);
