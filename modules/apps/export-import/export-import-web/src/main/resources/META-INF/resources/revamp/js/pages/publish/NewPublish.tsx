...
import {
	PublishPreviewParams,
	getPublishPreview,
} from '../../services/getPublishPreview';
...
export function NewPublish({
	...
	const [seeding, setSeeding] = useState(!!jobName);
	...
	const getPreview = useCallback(
		(publishPreviewParams: PublishPreviewParams) => {
			...
			getPublishPreview(publishPreviewParams).then(
	...
	useEffect(() => {
		getPreview({url: publishPreviewAPIURL});
	}, [publishPreviewAPIURL, getPreview]);

	useEffect(() => {
		if (!jobName || !preview || seededRef.current) {
			return;
		}
		...
		getScheduledPublishProcess({
			url: `${scheduledPublishProcessesAPIURL}/${encodeURIComponent(
				jobName
			)}`,
