...
public abstract class Base${schemaName}ResourceTestCase {
	...
	<#list javaMethodSignatures as javaMethodSignature>
		...
		<#elseif configYAML.generateGraphQL && freeMarkerTool.hasHTTPMethod(javaMethodSignature, "post") && (stringUtil.equals(javaMethodSignature.parentSchemaName!"", "") || stringUtil.equals(javaMethodSignature.parentSchemaName!"", "AssetLibrary") || stringUtil.equals(javaMethodSignature.parentSchemaName!"", "Site")) && javaMethodSignature.returnType?ends_with(schemaName) && !freeMarkerTool.hasRequestBodyMediaType(javaMethodSignature, "multipart/form-data")>
			<#assign postJavaMethodSignature = "" />

			<#if freeMarkerTool.hasJavaMethodSignature(javaMethodSignatures, "post" + (javaMethodSignature.parentSchemaName!"") + schemaName)>
				<#assign postJavaMethodSignature = freeMarkerTool.getJavaMethodSignature(javaMethodSignatures, "post" + (javaMethodSignature.parentSchemaName!"") + schemaName) />
			<#elseif stringUtil.equals(javaMethodSignature.parentSchemaName!"", "") && freeMarkerTool.hasJavaMethodSignature(javaMethodSignatures, "postSite" + schemaName)>
			...
			<#if postJavaMethodSignature?has_content>
				...
				public void testGraphQL${javaMethodSignature.methodName?cap_first}() throws Exception {
					...
					<#else>
						...
						${schemaName} ${schemaVarName} = testGraphQL${postJavaMethodSignature.parentSchemaName!""}${schemaName}_add${schemaName}(
		...
		<#if generateTestGraphQLAddMethod>
			<#assign
				generateSchemaGraphQLAddMethod = true
	...
	<#list parentSchemaNames as parentSchemaName>
		<#if stringUtil.equals(parentSchemaName, "") || stringUtil.equals(parentSchemaName, "AssetLibrary") || stringUtil.equals(parentSchemaName, "Site")>
			...
			<#if freeMarkerTool.hasJavaMethodSignature(javaMethodSignatures, "post" + parentSchemaName + schemaName)>
				<#assign
					effectiveParentSchemaName = parentSchemaName
					generateAppendGraphQLFieldValue = true
					postJavaMethodSignature = freeMarkerTool.getJavaMethodSignature(javaMethodSignatures, "post" + parentSchemaName + schemaName)
				/>
			<#elseif stringUtil.equals(parentSchemaName, "") && freeMarkerTool.hasJavaMethodSignature(javaMethodSignatures, "postSite" + schemaName)>
				<#assign
					effectiveParentSchemaName = "Site"
					generateAppendGraphQLFieldValue = true
					postJavaMethodSignature = freeMarkerTool.getJavaMethodSignature(javaMethodSignatures, "postSite" + schemaName)
				/>
			</#if>
				...
				protected ${schemaName} testGraphQL${parentSchemaName}${schemaName}_add${schemaName}(
					...
					return jsonDeserializer.deserialize(
						JSONUtil.getValueAsString(
							invokeGraphQLMutation(
								new GraphQLField(
									"create${effectiveParentSchemaName}${schemaName}",
									new HashMap<String, Object>() {
										{
											<#list postJavaMethodSignature.pathJavaMethodParameters as javaMethodParameter>
												<#if stringUtil.equals(javaMethodParameter.parameterName, "siteId")>
									...
									graphQLFields)),
							"JSONObject/data",
							"JSONObject/create${effectiveParentSchemaName}${schemaName}"),
						${schemaName}.class);
				}
			<#else>
				protected ${schemaName} testGraphQL${parentSchemaName}${schemaName}_add${schemaName}() throws Exception {
